	...
	<body>
		...
		<%
			/* 	
				获取json数据并解析
			*/
			// String apiListData = request.getParameter("apiListData"); //数据由转发到此页面的超链接中以参数形式给出
			...
			request.setAttribute("apiList",apiList); //将apiList设置在request域中,这样转发时可以获取
			request.setCharacterEncoding("utf-8"); //设置参数编码
		%>
		
	
		<!-- api列表,每一个api以表格形式展现,并用超链接包裹,点击可跳转至详情页面 -->
		...
		<script type="text/javascript">
			function jump(api_name){
				...
				// window.self.location.href="apiDetail.jsp?api_name="+api_name+"&apiListData="+param;
